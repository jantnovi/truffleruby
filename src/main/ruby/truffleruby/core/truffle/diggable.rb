...
module Truffle::Diggable
    ...
    def self.dig_fetch(obj, idxs)
      ...
      while n < idxs_size
        ...
        # We've inlined the logic for dig for three core classes - before we use that logic we need to check if the method has been monkey patched
        unless Primitive.vm_object_method_is_basic(obj, :dig_fetch)
          raise TypeError, "#{obj.class} does not have #dig_fetch method" unless obj.respond_to?(:dig_fetch)
          return obj.dig_fetch(*idxs[n...])
        end
